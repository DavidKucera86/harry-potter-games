...
| 2026-08-26 | **P — Purpose** | Coverage gate měřil jinou jednotku, než si všichni mysleli. Vitest 3 odvozoval pokrytí z v8 byte-ranges, takže každý řádek datového modulu byl „pokrytý statement" jen díky importu — `topics.ts` a `followUps.ts` přisypaly ~2300 statements na 100 %. Vitest 4 počítá podle AST: jmenovatel spadl 3692 → 1332 a reálné pokrytí logiky je **77,55 %**, ne 87,81 %. Pokrytí neklesalo, přestalo se ředit. Prahy překalibrované ze změřeného baseline. Typ mezery: **metrika, jejíž jednotku nikdo neověřil** — u čísla, které hlídá bránu, patří k procentu i to, co je ten jmenovatel zač, a ověří se to pohledem na jednotlivé soubory, ne na součet. |
