...
| 2026-08-22 | **P — Purpose** | První měření Lighthouse: performance 100, accessibility 100, seo 100, best-practices 96 na všech třech URL. Prahy zpřísněny na naměřené hodnoty. Best-practices drží na 96 jediná konzolová chyba: `frame-ancestors` se v `<meta>` CSP ignoruje a účinkuje jen jako HTTP hlavička. **Není to bezpečnostní díra** — hlavičku posílá `netlify.toml` i `docker/nginx.conf`, takže ochrana platí; meta kopie jen zbytečně loguje. Vypustit `frame-ancestors` z `META_CSP_HEADER` (ne z `CSP`) by dalo 100 a odstranilo chybu z konzole. Zásah do CSP → podle CLAUDE.md vyžaduje security pass, proto zatím jen zaznamenáno. |
